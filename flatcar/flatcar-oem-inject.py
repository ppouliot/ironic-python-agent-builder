...
def get_flatcar_version(version_file, key='FLATCAR_VERSION_ID'):
    with open(version_file) as f:
        for line in f:
            if line.startswith("%s=" % key):
                return line.strip().split("=")[1]
    raise ValueError('Could not read FlatCar Linux version number from file')

FLATCAR_VERSION_FILE = os.path.join(os.path.dirname(__file__), 'version.txt')
FLATCAR_VERSION = os.environ.get('FLATCAR_VERSION',
                                get_flatcar_version(FLATCAR_VERSION_FILE))

FLATCAR_ARCH = "amd64-usr"
FLATCAR_BASE_URL = ("http://stable.release.core-os.net/{arch}/{ver}"
                   .format(arch=FLATCAR_ARCH, ver=FLATCAR_VERSION))
FLATCAR_PXE_DIGESTS = "flatcar_production_pxe_image.cpio.gz.DIGESTS.asc"
FLATCAR_PXE_KERNEL = "flatcar_production_pxe.vmlinuz"
FLATCAR_PXE_IMAGE = "flatcar_production_pxe_image.cpio.gz"
FLATCAR_PXE_IMAGE_URL = "{url}/{img}".format(url=FLATCAR_BASE_URL,
                                            img=FLATCAR_PXE_IMAGE)
FLATCAR_PXE_KERNEL_URL = "{url}/{kernel}".format(url=FLATCAR_BASE_URL,
                                                kernel=FLATCAR_PXE_KERNEL)
FLATCAR_PXE_DIGESTS_URL = "{url}/{digests}".format(url=FLATCAR_BASE_URL,
                                                  digests=FLATCAR_PXE_DIGESTS)
...
def main():
    ...
    cache_dir = os.path.join(top_cache_dir, FLATCAR_ARCH, FLATCAR_VERSION)

    if not os.path.exists(cache_dir):
        os.makedirs(cache_dir)

    orig_cpio = os.path.join(cache_dir, FLATCAR_PXE_IMAGE)
    digests = os.path.join(cache_dir, FLATCAR_PXE_DIGESTS)
    kernel = os.path.join(cache_dir, FLATCAR_PXE_KERNEL)

    cache_file(digests, FLATCAR_PXE_DIGESTS_URL)
    gpg_verify_file(digests)
    cache_file(kernel, FLATCAR_PXE_KERNEL_URL)
    validate_digests(digests, kernel)
    cache_file(orig_cpio, FLATCAR_PXE_IMAGE_URL)
...
def gpg_verify_file(ascfile):
    ...
    try:
        ...
        key = os.path.join(d, 'flatcar.key')
